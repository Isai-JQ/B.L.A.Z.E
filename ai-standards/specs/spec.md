...
- RF-13: CUANDO un administrador de las impresoras de FrED-Factory reordene manualmente la cola, EL SISTEMA aplicará ese nuevo orden por encima de la prioridad automática para los trabajos afectados, sin importar la organización a la que pertenezca cada trabajo. Esto incluye el caso en que un reordenamiento entre trabajos de menor tier los deje por delante de un trabajo de FrED-Factory que no fue tocado por el reordenamiento — es el comportamiento esperado, confirmado explícitamente, no un error.
- RF-14: EL SISTEMA solo permitirá registrarse con correos del dominio `@tec.mx`; cualquier otro dominio se rechaza, tanto en el cliente (feedback inmediato) como en el servidor (para que no se pueda evadir llamando directo a la API).
